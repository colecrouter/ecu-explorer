...
const EFI_OBDI_PROFILE: Mut2ModuleProfile = {
	...
	pids: [
		buildPid(0x01, 0x00, "Battery Level", "V", 0, 16, [0x14], (bytes) => (bytes[0] ?? 0) * 0.07333),
		buildPid(0x01, 0x01, "Throttle Position", "%", 0, 100, [0x17], (bytes) => ((bytes[0] ?? 0) * 100) / 255),
		buildPid(0x01, 0x02, "Engine RPM", "rpm", 0, 8000, [0x21], (bytes) => (bytes[0] ?? 0) * 31.25),
		buildPid(0x01, 0x03, "Coolant Temp", "deg C", -40, 150, [0x10], (bytes) => (bytes[0] ?? 0) - 40),
...
const EFI_CEDDY_PROFILE: Mut2ModuleProfile = {
	...
	pids: [
		buildPid(0x02, 0x00, "Throttle Position", "%", 0, 100, [0x17], (bytes) => ((bytes[0] ?? 0) * 100) / 255),
		buildPid(0x02, 0x01, "Engine RPM", "rpm", 0, 8000, [0x21], (bytes) => (bytes[0] ?? 0) * 31.25),
		buildPid(0x02, 0x02, "Timing Advance", "deg", -20, 60, [0x06], decodeSignedOffset(1, -20)),
		buildPid(0x02, 0x03, "Load 1Byte", "load", 0, 357, [0x0a], (bytes) => ((bytes[0] ?? 0) * 5) / 4),
		buildPid(0x02, 0x04, "2Byte Load", "load", 0, 500, [0x00, 0x01], decodePair(5 / 16)),
		buildPid(0x02, 0x05, "2Byte RPM", "rpm", 0, 9000, [0x02, 0x03], decodePair(1000 / 256)),
	],
};
...
const ABS_PROFILE: Mut2ModuleProfile = {
	...
	supportsClearDtcs: true,
	pids: [
		buildPid(0x03, 0x00, "Battery Level", "V", 0, 16, [0x01], (bytes) => (bytes[0] ?? 0) / 10),
		buildPid(0x03, 0x01, "Front Right Wheel Speed", "km/h", 0, 255, [0x02], (bytes) => (bytes[0] ?? 0) * 0.57),
		buildPid(0x03, 0x02, "Front Left Wheel Speed", "km/h", 0, 255, [0x03], (bytes) => (bytes[0] ?? 0) * 0.57),
		buildPid(0x03, 0x03, "Rear Right Wheel Speed", "km/h", 0, 255, [0x04], (bytes) => (bytes[0] ?? 0) * 1.088),
...
const TCU_PROFILE: Mut2ModuleProfile = {
	...
	supportsClearDtcs: true,
	pids: [
		buildPid(0x04, 0x00, "Transmission Temperature", "deg C", 0, 255, [0x86], identity),
		buildPid(0x04, 0x01, "Switch Position P", "on/off", 0, 1, [0xb4], boolBit(0x40)),
		buildPid(0x04, 0x02, "Switch Position R", "on/off", 0, 1, [0xb4], boolBit(0x80)),
		buildPid(0x04, 0x03, "Switch Position N", "on/off", 0, 1, [0xb3], boolBit(0x01)),
		buildPid(0x04, 0x04, "Switch Position D", "on/off", 0, 1, [0xb3], boolBit(0x02)),
	],
};
...
const AYC_PROFILE: Mut2ModuleProfile = {
	...
	supportsClearDtcs: true,
	pids: [
		buildPid(0x05, 0x00, "Battery", "V", 0, 16, [0xa6], (bytes) => (bytes[0] ?? 0) / 10),
		buildPid(0x05, 0x01, "Throttle Position", "%", 0, 100, [0xa7], (bytes) => ((bytes[0] ?? 0) * 100) / 255),
		buildPid(0x05, 0x02, "Vehicle Speed", "km/h", 0, 255, [0xa4], identity),
		buildPid(0x05, 0x03, "G-Force Accel/Decel", "V", 0, 5, [0xa5], (bytes) => (2.5 * (bytes[0] ?? 0)) / 128),
		buildPid(0x05, 0x04, "Steering Angle Direction", "deg", -512, 512, [0xae], decodeSignedOffset(4, -512)),
	],
};
...
const ACD_PROFILE: Mut2ModuleProfile = {
	...
	supportsClearDtcs: true,
	pids: [
		buildPid(0x06, 0x00, "Battery", "V", 0, 16, [0x19], (bytes) => (bytes[0] ?? 0) / 10),
		buildPid(0x06, 0x01, "Pressure Switch", "on/off", 0, 1, [0x01], boolBit(0x01)),
		buildPid(0x06, 0x02, "Brake Switch", "on/off", 0, 1, [0x01], boolBit(0x10)),
		buildPid(0x06, 0x03, "Handbrake Switch", "on/off", 0, 1, [0x02], boolBit(0x01)),
		buildPid(0x06, 0x04, "Right AYC Valve Active", "on/off", 0, 1, [0x02], boolBit(0x40)),
	],
};
...
async function probeModule(
	...
	for (const probe of profile.probes) {
		try {
			const response = await connection.sendFrame(new Uint8Array([probe.requestId]));
			...
			while (running) {
				...
				for (const definition of selectedPids) {
					...
					try {
						...
						droppedFrames++;
						console.error(
							`[MUT-II] Failed to read ${definition.name}:`,
							error,
						);
					}
				}
				...
				const waitMs = Math.max(0, MUT2_POLL_INTERVAL_MS - (Date.now() - cycleStart));
				if (waitMs > 0) {
