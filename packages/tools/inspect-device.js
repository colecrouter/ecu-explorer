 ...
 * }} NodeUsbInterfaceDescriptor
 */

/**
 * @typedef {{
 *   endpoints?: NodeUsbEndpointLike[] | undefined;
 *   descriptor?: NodeUsbInterfaceDescriptor;
 *   isKernelDriverActive?: () => boolean;
 *   isClaimed?: boolean;
 *   claim: () => void;
 *   release: () => void;
 * }} NodeUsbInterfaceLike
 */
 ...
 *   };
 *   interfaces?: NodeUsbInterfaceLike[] | undefined;
 *   busNumber: number;
		...
		const usbInterface = {
			async getDevices() {
				try {
					...
					const devices = deviceList.map((nodeDevice) =>
						createWebUsbDevice(nodeDevice),
					);
					return /** @type {USBDevice[]} */ (devices);
				} catch (error) {
 ...
 *
 * @param {NodeUsbDeviceLike} nodeDevice - The node-usb device instance
 * @returns {USBDevice}
 */
function createWebUsbDevice(nodeDevice) {
	...
	function describeInterface(interfaceNumber) {
		try {
			const intf = nodeDevice.interface(interfaceNumber);
			const descriptor = intf?.descriptor || {};
			const kernelActive =
				typeof intf?.isKernelDriverActive === "function"
					? intf.isKernelDriverActive()
					: false;
			const endpoints = (intf?.endpoints || []).map(
				(/** @type {NodeUsbEndpointLike} */ ep) =>
					`${ep.address}:${ep.direction}/${ep.transferType}`,
			);
			return `iface=${descriptor.bInterfaceNumber ?? interfaceNumber} class=${
				descriptor.bInterfaceClass
			}/${descriptor.bInterfaceSubClass} proto=${descriptor.bInterfaceProtocol} kernelDriverActive=${kernelActive} endpoints=${endpoints.join(",")}`;
		} catch (_error) {
	...
	device.open = async () => {
		try {
			nodeDevice.open();
			// Refresh interface metadata after open so consumers can correctly inspect
			// interface descriptors and select the correct interface for this device.
			device.interfaces = nodeDevice.interfaces || [];
			device.opened = true;
		} catch (error) {
	...
	/** @param {string} direction @param {number} endpointNumber */
	function findEndpoint(direction, endpointNumber) {
		const interfaces = nodeDevice.interfaces || [];
		for (const usbInterface of interfaces) {
			const endpoints = usbInterface.endpoints || [];
			const endpoint = endpoints.find(
				(/** @type {NodeUsbEndpointLike} */ e) =>
					e.direction === direction && e.address === endpointNumber,
			);
			if (endpoint) {
				return endpoint;
			}
	...
	device.claimInterface = async (interfaceNumber) => {
		try {
			...
			const interfaceSummary = describeInterface(interfaceNumber);
			throw new Error(
				`Failed to claim interface ${interfaceNumber} (${interfaceSummary}): ${error}. ` +
					`This is usually macOS kernel ownership preventing libusb claims. ` +
					`Try WebUSB path or a host OS that can own the interface.`,
			);
		}
	};
	...
	device.transferIn = async (endpointNumber, length) =>
		new Promise((resolve, reject) => {
			...
			const endpoint = findEndpoint("in", endpointNumber);
	...
	device.transferOut = async (endpointNumber, data) =>
		new Promise((resolve, reject) => {
			...
			const endpoint = findEndpoint("out", endpointNumber);
			...
			endpoint.transfer(
				...
				(/** @type {Error | null} */ error, /** @type {unknown} */ result) => {
					...
					const transferred = Number(result ?? 0);
					resolve({
	...
	return /** @type {USBDevice} */ (/** @type {unknown} */ (device));
}

/**
 * @param {NodeUsbDeviceLike} device
 * @returns {string | null}
 */
